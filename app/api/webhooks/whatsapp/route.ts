...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const url = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/webhooks/whatsapp`
  ...
  if (!whatsappClient.validateWebhookSignature(body, signature, url)) {
  ...
  const incomingMessage = whatsappClient.parseIncomingMessage(req)
  if (!incomingMessage) {
    return NextResponse.json({ error: 'Invalid message format' }, { status: 400 })
  }

  // Get user from request (assuming user context from auth or config)
  // For now, we'll need to determine user from phone number or config
  // This might need adjustment based on your multi-tenant setup
  ...
  if (!chatbotResponse.ok) {
    console.error('PrismAI chatbot processing failed:', await chatbotResponse.text())
    return NextResponse.json({ error: 'PrismAI chatbot processing failed' }, { status: 500 })
  }
  ...
  const sendResult = await whatsappClient.sendMessage(incomingMessage.from, responseMessage)

  if (!sendResult.success) {
    console.error('Failed to send PrismAI WhatsApp response:', sendResult.error)
